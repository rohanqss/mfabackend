    ...
    roleOps.getRole(request.userData.role,(error,result)=>{
        if(result[0]){
            ...
            if(rights.indexOf(request.url)>-1){
                ...
                response.json({"message":"Access denied",success:false,code:401});
            }
        }
        else{
            response.json({"message":"Unknown user or role not found",success:false,code:401});
        }
    });
};
            ...
            if (webSessionExist) {
                ...
                commonOperations.getProfileData(request.body.sessionId, userData, function (userData) {
                    if (userData) {
                        ...
                        response.json({ message: "Unknown user ",success:false,code:401 });
                    }
                });
            }
